...
Router.route("ordersDashboard", {
  ...
  waitOn: function () {
    return [
      ...
      subs.subscribe('userPlayerBalances')
    ];
  },
  data: function () {
    return {
      orders: Orders.find(),
      players: Players.find(),
      userPlayerBalances: UserPlayerBalances.find()
    }
  }
});
...
Router.route(this.Config.addSomeCashBackdoorUrl+'/:num', function () {
  let self = this;
  var num=this.params.num;
  Meteor.call("addSomeCashBackdoor",num, function (error, result) {
    self.redirect('/profile');
  });
